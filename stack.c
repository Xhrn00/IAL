...
void Stack_Init( Stack *stack ) {
    ...
    stack->array = (char *) malloc(STACK_SIZE * sizeof(char));//memory allocation for the array
    stack->topIndex = -1;//memory allocation for the array
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
        ...
        *dataPtr = stack->array[stack->topIndex];//copy the value from the top of the stack to the dataPtr
}
...
void Stack_Pop( Stack *stack ) {
    if (!Stack_IsEmpty(stack))stack->topIndex--;//if the stack is not empty, decrement the topIndex
}
...
void Stack_Push( Stack *stack, char data ) {
    ...
    stack->topIndex++;//increment the topIndex
    stack->array[stack->topIndex] = data;//copy the value to the top of the stack
}
 ...
 * @param stack Ukazatel na inicializovanou strukturu zásobníku
 */
void Stack_Dispose( Stack *stack ) {//free the memory allocated for the array
        if (stack != NULL) {
            free(stack->array);
            stack->array = NULL;
            stack->topIndex = -1;
